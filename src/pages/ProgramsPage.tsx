    ...
    <div>
      <section className="relative bg-gradient-to-br from-primary-50 via-white to-purple-50 dark:from-slate-900 dark:via-slate-900 dark:to-indigo-950 py-20 md:py-28 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-6 lg:px-8 text-center">
          ...
          <h1 className="text-4xl md:text-6xl font-extrabold text-slate-900 dark:text-white mb-4">Explore Our Programs</h1>
          <p className="text-slate-500 dark:text-slate-400 text-lg max-w-2xl mx-auto">Industry-recognized tech training programs designed to kickstart your career in the digital world.</p>
        </div>
      </section>
      ...
      <section className="section-white py-16 lg:py-20">
        <div ref={s1.ref} className={`max-w-7xl mx-auto px-6 lg:px-8 animate-section ${s1.visible ? 'visible' : ''}`}>
          ...
          {loading ? (
            <div className="text-center py-20"><div className="w-12 h-12 border-[3px] border-slate-200 dark:border-slate-700 border-t-primary-600 rounded-full animate-spin mx-auto" /></div>
          ) : filteredPrograms.length === 0 ? (
            <div className="text-center py-20">
              <Search className="h-12 w-12 text-slate-300 dark:text-slate-600 mx-auto mb-4" />
              <h3 className="text-xl font-semibold text-slate-900 dark:text-white mb-2">No programs found</h3>
              ...
              {filteredPrograms.map((program, idx) => (
                <div key={program.id} className="card flex flex-col group" style={{ transitionDelay: `${idx * 60}ms` }}>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex items-center gap-1.5 text-slate-400 dark:text-slate-500 text-xs"><Clock className="h-3.5 w-3.5" />{program.duration}</div>
                  </div>
                  <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-3 group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors">{program.title}</h3>
                  <p className="text-slate-500 dark:text-slate-400 text-sm mb-5 flex-grow leading-relaxed line-clamp-3">{program.description}</p>
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-slate-100 dark:border-slate-700">
                    ...
                    <button onClick={() => navigate('contact')} className="flex items-center gap-1 text-primary-600 dark:text-primary-400 text-sm font-semibold group-hover:gap-2 transition-all">
      ...
      <section className="section-gray py-16 lg:py-20">
        <div ref={s2.ref} className={`max-w-4xl mx-auto px-6 lg:px-8 text-center animate-section ${s2.visible ? 'visible' : ''}`}>
          <div className="card !p-10 !border-primary-100 dark:!border-primary-500/20 bg-gradient-to-br from-white to-primary-50/50 dark:from-slate-800 dark:to-primary-900/20">
            <BookOpen className="h-10 w-10 text-primary-500 dark:text-primary-400 mx-auto mb-4" />
            <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-3">Not sure which program is right for you?</h2>
            <p className="text-slate-500 dark:text-slate-400 mb-6 max-w-xl mx-auto">Reach out to us and our team will help you find the perfect program based on your skills and career goals.</p>
